...
class ProductDataRelevanceModifier extends RelevanceModifier
{
     ...
     * @param ValueCondition[] $conditions The conditions that must hold for the specific data Key in order for the product to be boosted.
     * @param ValueSelector $multiplierSelector The selector for the multiplier which the products parsing the Conditions will be have their rank multiplied by. It can either be a FixedDoubleValueSelector taking a fixed value or a DataDoubleSelector that can take the multiplier from a data key containing a double.
     * @param bool $mustMatchAllConditions Specifies whether all Conditions should parse their test on the specific data Key () or if only one is required ().
     * @param bool $considerAsMatchIfKeyIsNotFound Specifies whether products that don't have the specific data Key should be considered a match () or not ().
     */
