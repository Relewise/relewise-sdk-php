...
class DisplayAdDataFilter extends DataFilter
{
    public string $typeDefinition = "Relewise.Client.Requests.Filters.DisplayAdDataFilter, Relewise.Client";
    public static function create(string $key, bool $filterOutIfKeyIsNotFound = true, ?Language $language = Null, ?Currency $currency = Null) : DisplayAdDataFilter
    {
        $result = new DisplayAdDataFilter();
        $result->key = $key;
    ...
    public static function hydrate(array $arr) : DisplayAdDataFilter
    {
        $result = DataFilter::hydrateBase(new DisplayAdDataFilter(), $arr);
        return $result;
    }
